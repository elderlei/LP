...
//Verificação do Sudoku por varredura, elemento a elemento
//Pega um elemento da matriz e compara com toda a linha e toda a sua coluna para verificar repetições
// Depois subdivide em matrizes 3x3 e faz o mesmo
...
int teste_submatriz(int matrix[9][9],int lin,int col,int l1,int l2,int c1,int c2){
  ...
  //comparação do elemento enviado para a função com todos os demais na mesma submatriz
  for(i=l1;i<l2+1;i++){
    for(j=c1;j<c2+1;j++){
      if(matrix[lin][col]==matrix[i][j]){
      count++;
      }
    }
  }
  // se não houver repetição, a contagem total deve ser =1, que é a identificação do elemento com ele mesmo nas varreduras
  if (count>1) return 1;
  else return 0;
}
